...
const EditCategoryForm = ({todos, defaultCategory, closeForm}: Props) => {
  const {
    register,
    handleSubmit,
    formState: {errors},
  ...
  const mutation = useMutation(
    (newCategory: CategoryFormProps) => {
      return axios.put(`/api/category/put/${defaultCategory.id}`, newCategory);
    },
    {
      onSuccess: () => {
  ...
  return (
    <div className='bg-slate-700 text-center shadow-md rounded-md my-3 px-2 py-3'>
      ...
      <h3 className='text-white text-2xl font-bold p-3 my-2'>Edit Category</h3>
